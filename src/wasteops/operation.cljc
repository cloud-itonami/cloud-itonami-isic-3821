(ns wasteops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (WasteOpsAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the WasteTreatmentOpsGovernor
  ...
  human operator/facility supervisor. The approver resumes with
  `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [wasteops.advisor :as advisor]
            [wasteops.governor :as governor]
...
(defn build
  ...
  `wasteops.store/Store`).
  opts:
    :advisor      -- a `wasteops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; WasteOpsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; WasteTreatmentOpsGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
